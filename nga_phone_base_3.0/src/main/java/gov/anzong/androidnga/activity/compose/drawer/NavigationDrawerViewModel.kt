...
import gov.anzong.androidnga.activity.compose.board.HiddenBoardFragment
...
class NavigationDrawerViewModel : ViewModel() {
    ...
    /** 路由常量在 base 模块，与本文件已导入的 app 版 ARouterConstants 同名，这里写全限定名 */
    fun startHiddenBoardPage(context: Context) {
        ARouterUtils
            .build(com.justwen.androidnga.base.activity.ARouterConstants.ACTIVITY_FRAGMENT_TEMPLATE)
            .withString("fragment", HiddenBoardFragment::class.java.name)
            .withString("title", "隐藏夹")
            .navigation(context)
    }
